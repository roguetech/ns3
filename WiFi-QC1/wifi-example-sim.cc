...
int main (int argc, char *argv[]) {
  ...
  sender->SetAttribute("Interval", StringValue ("ns3::ConstantRandomVariable[Constant=0.004]")); //seconds
  ...
  Ptr<Sender> sender1 = CreateObject<Sender>();
  sender1->SetAttribute("Port", UintegerValue(1001));//Lisening Port of the first WiFi user
  sender1->SetAttribute("PacketSize", UintegerValue (1000)); //bytes
  sender1->SetAttribute("Interval", StringValue ("ns3::ConstantRandomVariable[Constant=0.004]")); //seconds
  sender1->SetAttribute("NumPackets",UintegerValue (100000000));
  appSource1->AddApplication (sender1);
  sender1->SetStartTime (Seconds (0));
  ...
  Ptr<Node> appSink1 = NodeList::GetNode (2);
  Ptr<Receiver> receiver1 = CreateObject<Receiver>();
  receiver1->SetAttribute("Port", UintegerValue(1001));//Lisening Port
  appSink1->AddApplication (receiver1);
  ...
  totalRx1->SetKey ("wifi-rx-frames1");
  totalRx1->SetContext ("node[1]");
  ...
  Ptr<PacketCounterCalculator> totalRx2 =
    CreateObject<PacketCounterCalculator>();
  totalRx2->SetKey ("wifi-rx-frames2");
  totalRx2->SetContext ("node[2]");
  Config::Connect ("/NodeList/2/DeviceList/*/$ns3::WifiNetDevice/Mac/MacRx",
                   MakeCallback (&PacketCounterCalculator::PacketUpdate,
                                 totalRx2));
  dataofuser1.AddDataCalculator (totalRx2);
  ...
  appRx1->SetKey ("receiver-rx-packets1");
  appRx1->SetContext ("node[1]");
  receiver->SetCounter (appRx1);
  dataofuser1.AddDataCalculator (appRx1);

  Ptr<CounterCalculator<> > appRx2 =
    CreateObject<CounterCalculator<> >();
  appRx2->SetKey ("receiver-rx-packets2");
  appRx2->SetContext ("node[2]");
  receiver1->SetCounter (appRx2);
  dataofuser1.AddDataCalculator (appRx2);
  /**
