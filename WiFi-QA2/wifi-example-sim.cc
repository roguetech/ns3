...
int main (int argc, char *argv[]) {
  ...
  cmd.AddValue ("distance", "Distance apart to place nodes (in meters).",
                distance);
  cmd.AddValue ("interval", "Interval between packets to increase bitrate.",
		  interval);
  ...
  if (interval == "0.05") {
	  ...
	  bitrate = "1";
  } else if (interval == "0.005333333") {
	  bitrate = "1.5";
  } else if (interval == "0.0016") {
	  bitrate = "5";
  } else if (interval == "0.0008") {
	  bitrate = "10";
  } else if (interval == "0.0004") {
	  bitrate = "20";
  }
  ...
  sender->SetAttribute("Interval", StringValue ("ns3::ConstantRandomVariable[Constant=" + interval + "]")); //seconds
  ...
  DataCollector DBQA2;
  DBQA2.DescribeRun (experiment,
                    strategy,
                    inputdistance,
                    runID);

  // Add any information we wish to record about this run.
  DBQA2.AddMetadata ("author", "EE500-PatrickRoughan"); //Please replace XXX with your first name!
  ...
  DBQA2.AddDataCalculator (totalTx);
  ...
  Config::Connect ("/NodeList/1/DeviceList/*/$ns3::WifiNetDevice/Mac/MacRx",
                   MakeCallback (&PacketCounterCalculator::PacketUpdate,
                                 totalRx));
  DBQA2.AddDataCalculator (totalRx);
  ...
  **/
  ...
  Config::Connect ("/NodeList/0/ApplicationList/*/$Sender/Tx",
                   MakeCallback
                     ...
                     appTxPkts));
  DBQA2.AddDataCalculator (appTxPkts);
